 ...
 *  2. Registers one [M3UPlaylistPlayerProvider] per saved playlist.
 *  3. Opens [M3UPlaylistPlayerSettings] when the user taps the plugin settings icon.
 */
@CloudstreamPlugin
class M3UPlaylistPlayerPlugin : Plugin() {
    ...
    override fun load(context: Context) {
        ...
        getSavedPlaylists(prefs).forEach { (name, url) ->
            registerMainAPI(M3UPlaylistPlayerProvider(customName = name, playlistUrl = url))
        }

        // Show settings UI when user opens settings for this plugin
        val act = context as AppCompatActivity
        openSettings = {
            M3UPlaylistPlayerSettings(
                plugin = this,
                prefs  = prefs,
                onPlaylistsChanged = { refreshProviders(context) }
            ).show(act.supportFragmentManager, "M3UPlaylistPlayerSettings")
        }
    }
    ...
    companion object {
        const val PREF_NAME      = "M3UPlaylistPlayerProvider"
